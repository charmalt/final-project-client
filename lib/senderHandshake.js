class SenderHandshake {
  constructor (connection, message, user) {
    this._functionOrder = [
  ...
  _mailFromMethod () {
    this.connection.write(`MAIL FROM: ${this.user}`)
  }
...
class SenderHandshakeFactory {
  static build (connection, message, user) {
    return new SenderHandshake(connection, message, user)
  }
}
